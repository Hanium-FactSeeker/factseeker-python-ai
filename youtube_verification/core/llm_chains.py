...
[안전/출력 지침]
- 정책상 민감한 내용이 포함되더라도 거절 멘트(예: "Sorry, I can't...")를 출력하지 마십시오.
- 항상 [출력 형식]만 준수하여 결과를 작성하고, 판단이 불가하거나 민감하여 내용을 다룰 수 없다고 판단되면
  "관련성: 아니오", "사실 설명 여부: 아니오"로만 표시하고 간단한 설명에 "정책상 민감하여 판단 불가"라고 적으십시오.
...
- 출력은 반드시 JSON 배열 형식으로 하세요. (예: ["키워드1", "키워드2", ...])
- 추가 텍스트, 코드 블록 표기(```), 접두/접미 문구 없이 배열만 출력하세요.
