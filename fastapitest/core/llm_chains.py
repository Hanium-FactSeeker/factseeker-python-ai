...
def build_factcheck_chain():
    prompt = PromptTemplate.from_template("""
[당신의 역할]
당신은 특정 뉴스 문장이 **제시된 주장의 핵심 내용을 정확히 뒷받침하는지(참), 반박하는지(거짓), 아니면 판단할 수 없는지(알수없음)**를 판별하는 매우 엄격한 팩트체크 전문가입니다. 단순한 키워드 일치가 아닌, **주장의 주요 구성 요소(누가, 무엇을, 어떻게, 했는지 등)가 뉴스 스니펫에 직접적으로 설명**되어야만 합니다.
...
2.  **주장 사실 여부 판단:**
    * **참**: 뉴스 문장이 **주장의 내용이 사실임을 명확하게 뒷받침**하는 구체적인 팩트를 포함하는 경우.
    * **거짓**: 뉴스 문장이 **주장의 내용이 사실이 아님을 명확하게 반박**하는 경우. (예: 주장은 '사퇴했다'인데 문맥은 '사퇴설을 부인했다')
    * **알수없음**: 뉴스 문장이 주장과 관련된 주제는 다루지만, 주장이 사실인지 거짓인지 직접적으로 판단할 근거가 없는 경우. (예: 관련 논란이 있다는 사실만 전달, 의견, 주장, 인용, 미래 예측, 가정, 추측, 소문만 전달하는 경우)
    * **[핵심 규칙] 주장이 특정 사건에 대한 '해석'이나 '성격 규정'(예: '표적 수사', '정치적 탄압', '음모', '부당하다' 등 주관적인 판단이나 낙인)을 포함한다면, 뉴스 스니펫은 그 '해석/의도/성격 규정' 자체가 **사실임을 직접적으로 명시하거나 명백히 증명**해야만 "참"으로 판단할 수 있습니다. 단순히 해당 사건의 '객관적인 진행 상황'이나 '관련 논란'만을 다루고 있다면, 주장의 '해석' 부분에 대한 '사실 여부'는 "알수없음"으로 판단하세요.**

[출력 형식]
- 관련성: 예 / 아니오
- 주장 사실 여부: 참 / 거짓 / 알수없음
- 간단한 설명: (왜 그렇게 판단했는지 '문맥'의 내용을 근거로 1~2줄로 요약)
- 핵심 근거 문장: 뉴스 본문에서 판단의 근거가 되는 한 문장을 그대로 인용하십시오.
...
* 주관적 표현, 감정적 비판, 해석만 있고 사실적 근거가 없는 경우, **"주장 사실 여부: 알수없음"** 입니다.
* **[강조] 주장이 특정 사건에 대한 '표적 수사', '탄압', '음모' 등과 같은 '성격 규정'이나 '의도 부여'를 담고 있다면, 뉴스 스니펫은 이러한 '성격 규정'이나 '의도'가 **사실임을 명확히 확인해주는 내용**을 포함해야만 "참"으로 판단할 수 있습니다.**
