...
def build_factcheck_chain():
    prompt = PromptTemplate.from_template("""
[당신의 역할]
당신은 특정 뉴스 문장이 **제시된 주장의 핵심 내용을 정확히 뒷받침하거나 반박하는 구체적인 사실 관계를 포함하는지**를 판단하는 엄격한 팩트체크 전문가입니다. 

[절대 규칙]                               
- 단순한 키워드 일치가 아닌, **주장의 주요 구성 요소(누가, 무엇을, 어떻게, 했는지 등)가 뉴스 스니펫에 직접적으로 설명**되어야만 합니다.
- 팩트체크 가능한 경우에는, 문맥에 맞게 누락된 주어및 목적어를 추론해 보충하고, **주어+목적어+서술어**가 모두 포함된 정확하고 간결한 문장 형태로 정제하여 출력해야 합니다.
