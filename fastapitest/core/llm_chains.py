...
def build_claim_extractor():
    """
    팩트체크 가능한 주장을 추출하고 정제하는 프롬프트를 생성하는 함수
    """
...
# ──────────────────────────────────────────────
# [최상위 핵심 원칙 (Master Rule)]
# ──────────────────────────────────────────────
# 1. [검색 최적화] 주장은 '압박', '기류', '무력화' 같은 추상적 서술이 아닌, **검색 가능한 구체적 '사건', '발언', '수치'**를 중심으로 구성해야 합니다. (가장 중요)
# 2. [단일 주장 분리] 하나의 문장에 여러 주장이 있다면, 각각의 주장을 별개의 문장으로 분리하여 개별적으로 판단하고 출력해야 합니다.
# 3. [출력 형식 준수] 아래 [출력 형식] 섹션에 명시된 포맷을 다른 모든 지침에 우선하여 반드시 지켜주세요.

[판단 목적]
- 문장이 참인지 거짓인지를 따지는 것이 아니라, **객관적 사실 여부를 검증할 수 있는 주장인지**를 선별하는 것이 목표입니다.
- 팩트체크 가능한 경우에는, 문맥에 맞게 누락된 주어 및 목적어를 추론해 보충하고, **주어+목적어+서술어**가 모두 포함된 정확하고 간결한 문장 형태로 정제하여 출력해야 합니다.
...
- 감정, 해석, 비난, 주관적 평가 중심인 경우 (예: '방통위가 무력화되고 있다', '압박 기류가 흐른다')
...
- **주어 없는 주장 처리**: 
    - 자막에 주어가 없는 수동태 문장("~라는 의혹을 받고 있다")이나 서술절("~라는 사실이 드러났다")의 경우, 맥락상 의심이나 행위의 주체를 파악하여 **"(특정 주체는)"** 또는 **"(현 정부는)"** 과 같이 주어를 명시해야 합니다.
    - 맥락 파악이 불가능하면 **"(발언자는)"** 을 주어로 삼아 "~라고 주장했다"의 형태로 정제할 수 있습니다.
    - 단, 이 규칙은 [최상위 핵심 원칙]의 '없는 정보 추가 금지' 원칙을 위배하지 않는 선에서만 적용됩니다.
...
# 다른 어떤 텍스트도 없이, 아래 형식만 정확히 지켜서 출력하세요.
...
# [검색 최적화를 위한 예시]
# 원본 자막: "정권이 바뀌면서 방통위를 완전히 무력화시키려는 거예요. 실제로 국무회의에서도 쫓아냈잖아요."
# 잘못된 추출 ❌: "방통위가 정권 교체와 함께 무력화되고 있다." → (팩트체크 불가능): 추상적 표현
# 올바른 추출 ✅: "이재명 정부가 이진숙 방통위원장을 국무회의에서 배석 배제했다." → 팩트체크 가능

- [1] 김민석은 국가 채무 비율을 20~30%로 착각하고 있다. → (부분 가능): 발언 여부는 검증 가능하나 표현은 수사적임
- [2] 이재명 정부가 방송 3법 개정안을 추진하고 있다. → 팩트체크 가능
- [3] 이진숙 방통위원장이 경찰에 출석해 조사를 받았다. → 팩트체크 가능
- [4] 김민석은 정치적으로 죽었다. → 팩트체크 불가능 (주관적 표현 + 검증 불가)
