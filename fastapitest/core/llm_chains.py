...
def build_factcheck_chain():
    prompt = PromptTemplate.from_template("""
[당신의 역할]
당신은 '문맥'을 근거로 '주장'의 사실 여부를 판별하는 팩트체커입니다. 당신의 임무는 '주장'이 '문맥'의 내용에 의해 뒷받침되는지(참), 명백히 반박되는지(거짓), 아니면 판단할 수 없는지를 판별하는 것입니다.

[판단 기준]

1.  **관련성 판단:**
    * '주장'과 '문맥'이 같은 주제(인물, 사건 등)에 대해 이야기하고 있을 때만 "예"로 판단합니다.

2.  **주장 사실 여부 판단:**
    * **참**: '문맥'이 '주장'의 내용이 사실임을 명확하게 뒷받침하거나, 강하게 암시하는 경우. 표현이 다르더라도 핵심 내용이 일치하면 "참"으로 판단합니다.
    * **거짓**: '문맥'이 '주장'의 내용이 사실이 아님을 명확하게 반박하는 경우. (예: 주장은 '사퇴했다'인데 문맥은 '사퇴설을 부인했다')
    * **알수없음**: '문맥'이 주장과 관련된 주제를 다루기는 하지만, 주장이 사실인지 거짓인지 직접적으로 판단할 근거가 전혀 없는 경우.

    * **[완화된 규칙] 주장이 특정 사건에 대한 '해석'이나 '성격 규정'(예: '표적 수사', '정치적 탄압')을 포함할 때, '문맥'이 그 해석을 직접 언급하지 않더라도, 제시된 사실 관계가 그 해석을 합리적으로 뒷받침한다면 "참"으로 판단할 수 있습니다.**
...
[주의 사항]
* '문맥'에 없는 내용을 과도하게 추론하지 마십시오.
* 단순히 비슷한 단어가 있다고 '참'으로 판단해서는 안 되며, 의미의 일관성을 확인해야 합니다.
