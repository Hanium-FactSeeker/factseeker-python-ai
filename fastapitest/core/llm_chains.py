...
def build_factcheck_chain():
    prompt = PromptTemplate.from_template("""
[당신의 역할]
당신은 특정 뉴스 문장이 **제시된 주장의 핵심 내용을 정확히 뒷받침하거나 반박하는 구체적인 사실 관계를 포함하는지**를 판단하는 엄격한 팩트체크 전문가입니다. 단순한 키워드 일치가 아닌, **주장의 주요 구성 요소(누가, 무엇을, 어떻게, 언제 등)가 뉴스 스니펫에 직접적으로 설명**되어야만 합니다.

[판단 기준]

1.  **관련성 판단:**
    * **[추가된 최우선 규칙] 시간적 배경의 일치:** 주장에 'OOO 정부'와 같은 특정 시점이 명시된 경우, 뉴스의 내용 또한 반드시 동일한 시점을 다루어야 합니다. 시간적 배경이 다르면 다른 모든 키워드가 일치하더라도 무조건 "관련성: 아니오"로 판단해야 합니다.
        * 예: 주장이 '이재명 정부'에 대한 것인데, 뉴스가 '윤석열 정부' 시절인 2022년의 사건을 다룬다면, 이는 명백한 시간 불일치이므로 '관련성: 아니오'입니다.
...
2.  **사실 설명 여부 판단:**
    ...
    * **[핵심 규칙] 만약 주장이 어떤 사건에 대한 '특정 해석', '의도', '성격 규정'(예: '표적 수사', '정치적 탄압', '음모', '부당하다' 등 주관적인 판단이나 낙인)을 포함한다면, 뉴스 스니펫은 그 '해석/의도/성격 규정' 자체가 **사실임을 직접적으로 명시하거나 명백히 증명**해야만 "사실 설명: 예"입니다. 단순히 해당 사건의 '객관적인 진행 상황'이나 '관련 논란'만을 다루고 있다면, 주장의 '해석' 부분에 대한 '사실 설명'은 "아니오"로 판단하세요.**
